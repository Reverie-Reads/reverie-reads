...
reset role;
select is((select array[pub_y,pub_m,pub_d]::int[] from public.works where id = 'ab000000-0000-4000-8000-000000000002'), array[2010,null,null], 'does not append another source month/day');
select is((select array[pub_y,pub_m,pub_d]::int[] from public.works where id = 'ab000000-0000-4000-8000-000000000003'), array[2024,2,29], 'all incoming axes travel together');
...
select is((select array[pub_y,pub_m,pub_d]::int[] from public.works where id='ab000000-0000-4000-8000-000000000004'), array[2010,null,null], 'preservation does not extend a different source date');
select is((select array[pub_y,pub_m,pub_d]::int[] from public.works where id='ab000000-0000-4000-8000-000000000005'), array[2024,2,29], 'preservation takes all valid incoming axes together');
set local role authenticated;
...
select lives_ok($q$select public.merge_books_authoritative('ab000000-0000-4000-8000-000000000008','ab000000-0000-4000-8000-000000000009','{"pub_y":2025,"pub_m":null,"pub_d":null}')$q$, 'an explicit duplicate choice accepts year precision');
